    ...
    def convert(self, data):
        ...
        for section in ('attributes', 'timeseries'):
            for config in self.__config.get(section, []):
              ...
              # -----------------------------------------------------------------------------------------------------------   
                    group_data = data.get(config.get('groupAddress'))

                    if not group_data:
                        self.__log.warning(
                            'No data received for group address %s',
                            config.get('groupAddress')
                        )
                        continue

                    converted_value = group_data.get('response')
                    if hasattr(converted_value, 'value'):
                        converted_value = converted_value.value

                    if isinstance(converted_value, str):
                        normalized_value = converted_value.upper()

                        if normalized_value in ('ON', 'TRUE', '1'):
                            converted_value = True
                        elif normalized_value in ('OFF', 'FALSE', '0'):
